"""Challenge 07 - Vulnerable stored prompt injection via SQLi."""
...
APP_NAME = "Challenge 07: SQL Stored Prompt (Vulnerable)"
INSTRUCTIONS = (
    "Triages service desk tickets. Allows operators to write tickets and "
    "summarize them for an AI analyst. Data stored in SQLite must be trusted."
)
...
log = logging.getLogger("challenge07.vulnerable")
...
def create_ticket(author: str, content: str) -> str:
    """Intentionally vulnerable ticket creation using string interpolation."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    query = f"INSERT INTO tickets (author, content) VALUES ('{author}', '{content}')"
    cursor.execute(query)  # nosec - intentional SQL injection flaw
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    ...
    log.info("Starting vulnerable SQL stored prompt server on %s:%s", host, port)
